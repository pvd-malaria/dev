...
setDT(c19)
c19[,count_19 := readr::parse_number(c19$count, locale = readr::locale(grouping_mark = "."))
    ][,count := NULL]
c19
...
pop_total <- pop_aml %>% number(big.mark = ".",
                                decimal.mark = ",")
pop_percent <- (pop_aml/pop_br) %>% percent(big.mark = ".",
                                          decimal.mark = ",")
...
# export tables that generated graphs -----------------------------------------
fwrite(pop2, "atlas/01_volume_populacao.csv", sep = ";", dec = ",")
fwrite(pop3, "atlas/01_crescm_populacao.csv", sep = ";", dec = ",")
