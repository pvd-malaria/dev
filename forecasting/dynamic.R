...
malaria2 <- malaria %>%
  mutate(apm = casos / populacao,
         area_desmat = incremento) %>%
  select(-c(casos, populacao, incremento))

# autoplot(malaria2, apm)

# Lambda value for boxcox transformation

lambda <- guerrero(malaria2$apm)
...
avgs <- malaria_tr %>%
  index_by(mes2 = month(mes, label = TRUE)) %>%
  group_by(uf) %>%
  summarise(prec        = mean(prec, na.rm = TRUE),
            tmax        = mean(tmax, na.rm = TRUE),
            tmin        = mean(tmin, na.rm = TRUE),
            area_desmat = mean(area_desmat, na.rm = TRUE))

malaria_ts <- malaria2 %>%
  filter_index("2019-01" ~ "2019-12") %>%
  mutate(prec = avgs$prec,
         tmax = avgs$tmax,
         tmin = avgs$tmin,
         area_desmat = avgs$area_desmat)
...
jd_accuracy <- function(fit, window_length = "1m") {
  ...
  fit %>%
    ...
    print(n = Inf) %>%
    gt::gt() %>%
    gt::gtsave(paste(uf, "_tscv_", window_length, "_acc.png"))
}
